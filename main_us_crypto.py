...
# --- Configuration ---
# Load both US and Crypto tickers
try:
    US_TICKERS = pd.read_csv('TICKERS_US.csv')['ticker'].tolist()
    print(f"Loaded {len(US_TICKERS)} US tickers from TICKERS_US.csv")
except FileNotFoundError:
    print("TICKERS_US.csv not found. Using default US list.")
    US_TICKERS = ["DJI", "INX"]

try:
    CRYPTO_TICKERS = pd.read_csv('TICKERS_CRYPTO.csv')['ticker'].tolist()
    print(f"Loaded {len(CRYPTO_TICKERS)} crypto tickers from TICKERS_CRYPTO.csv")
except FileNotFoundError:
    print("TICKERS_CRYPTO.csv not found. Using default crypto list.")
    CRYPTO_TICKERS = ["BTC", "ETH"]

# Combine all tickers for processing
TICKERS_TO_DOWNLOAD = US_TICKERS + CRYPTO_TICKERS

# Define the names for your data and report directories.
# These can be modified by the --week flag in main().
DATA_DIR = "market_data_us_crypto"
REPORTS_DIR = "reports_us_crypto"
MASTER_REPORT_FILENAME = "REPORT_us_crypto.md"
VPA_ANALYSIS_FILENAME = "VPA_us_crypto.md"
...
def get_data_source_for_ticker(ticker):
    """
    Determine if ticker is US index or crypto and return appropriate vnstock object.
    """
    if ticker in US_TICKERS:
        return Vnstock().world_index(symbol=ticker, source='MSN'), 'US_INDEX'
    elif ticker in CRYPTO_TICKERS:
        return Vnstock().crypto(symbol=ticker, source='MSN'), 'CRYPTO'
    else:
        raise ValueError(f"Unknown ticker type: {ticker}")

def download_stock_data(ticker, start_date, end_date, interval='1D'):
    """
    Checks for local data first. If not found, downloads historical data using 
    appropriate vnstock method (crypto or world_index).
    ...
    try:
        # Get appropriate vnstock object for this ticker
        data_reader, ticker_type = get_data_source_for_ticker(ticker)
        
        df = data_reader.quote.history(
            start=start_date,
            end=end_date,
            interval=interval
        )
        ...
        if not df.empty:
            print(f"   - Success! Downloaded {len(df)} raw records for {ticker} ({ticker_type}).")
            df['time'] = pd.to_datetime(df['time'])
...
def parse_vpa_analysis(file_path):
    """
    Parses the VPA_us_crypto.md file to extract analysis for each ticker, preserving indentation.
    
    Args:
        file_path (str): The path to the VPA_us_crypto.md file.
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            # Check for a ticker header (e.g., "# BTC")
...
def generate_candlestick_report(df, ticker, start_date, end_date, interval):
    ...
    # Determine asset type for title
    asset_type = "US Index" if ticker in US_TICKERS else "Cryptocurrency"

    mpf.plot(
        plot_df,
        type='candle', style=style,
        title=f"{ticker} ({asset_type} - {timeframe}) from {start_date} to {end_date}",
...
def generate_master_report(report_data, vpa_analyses, ticker_groups, ticker_to_group_map, start_date, end_date):
    """
    Generates an improved master REPORT_us_crypto.md file with a Table of Contents and deep links.
    ...
    with open(MASTER_REPORT_FILENAME, 'w', encoding='utf-8') as f:
        # --- Main Header ---
        f.write("# AIPriceAction US & Crypto Market Report\n")
        f.write(f"*Report generated for data from **{start_date}** to **{end_date}**.*\n")
        f.write(f"*Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*\n\n")
        
        # --- Asset Categories ---
        f.write("---\n\n")
        f.write("## 📈 Asset Categories\n\n")
        f.write("**US Indices:**\n")
        us_links = [f"[{t}](#{t.lower()})" for t in US_TICKERS if any(rd['ticker'] == t for rd in report_data)]
        f.write(', '.join(us_links) + "\n\n")
        
        f.write("**Cryptocurrencies:**\n")
        crypto_links = [f"[{t}](#{t.lower()})" for t in CRYPTO_TICKERS if any(rd['ticker'] == t for rd in report_data)]
        f.write(', '.join(crypto_links) + "\n\n")
        f.write("---\n\n")
        ...
        f.write("| Ticker | Type | Actions |\n")
        f.write("|:-------|:-----|:--------|\n")
        for data in report_data:
            ticker_id = data['ticker'].lower() # Markdown anchors are typically lowercase
            asset_type = "US Index" if data['ticker'] in US_TICKERS else "Crypto"
            # Add a direct link to the CSV data file
            f.write(f"| **[{data['ticker']}](#{ticker_id})** | {asset_type} | [[Download CSV]({data['csv_path']})] |\n")
        f.write("\n---\n\n")

        # --- Summary Table ---
        f.write("## Asset Performance Summary\n")
        f.write("| Ticker | Type | Period High | Period Low | Latest Close | Change % | Total Volume |\n")
        f.write("|:-------|:-----|------------:|-----------:|-------------:|---------:|-------------:|\n")
        for data in report_data:
            asset_type = "US Index" if data['ticker'] in US_TICKERS else "Crypto"
            ...
            f.write(
                f"| **{data['ticker']}** | {asset_type} | {data['period_high']:,} | {data['period_low']:,} | **{data['latest_close']:,}** | "
        ...
        for data in report_data:
            ...
            asset_type = "US Index" if data['ticker'] in US_TICKERS else "Cryptocurrency"
            f.write(f"### {data['ticker']} ({asset_type})\n\n")
            ...
            # --- Back to Top link ---
            f.write('<p align="right"><a href="#vpa-signal-summary">↑ Back to Top</a></p>\n\n')
            ...
            f.write(f"| **Latest Close** | **{data['latest_close']:,} USD** |\n")
            f.write(f"| Period Open | {data['period_open']:,} USD |\n")
            f.write(f"| Period High | {data['period_high']:,} USD |\n")
            f.write(f"| Period Low | {data['period_low']:,} USD |\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="AIPriceAction US & Crypto Data Pipeline")
    ...
    if args.week:
        ...
        DATA_DIR = "market_data_us_crypto_week"
        REPORTS_DIR = "reports_us_crypto_week"
        MASTER_REPORT_FILENAME = "REPORT_us_crypto_week.md"
        VPA_ANALYSIS_FILENAME = "VPA_us_crypto_week.md"
        data_interval = '1W'

    START_DATE = args.start_date
    END_DATE = args.end_date

    print("--- AIPriceAction US & Crypto Data Pipeline: START ---")
    ...
    try:
        with open('ticker_group_us_crypto.json', 'r', encoding='utf-8') as f:
            ticker_groups = json.load(f)
        print("Loaded ticker groups from ticker_group_us_crypto.json")
    except FileNotFoundError:
        print("ticker_group_us_crypto.json not found. Skipping group section.")
        ticker_groups = {}
    ...
    print("\n--- AIPriceAction US & Crypto Data Pipeline: FINISHED ---")
