#!/usr/bin/env python3
"""
Script to analyze VPA data preparation for weekly analysis
"""

import csv
import os
import re
from datetime import datetime, timedelta

def read_csv_data(ticker, market_data_dir, target_date):
    """Read OHLCV data for a ticker from weekly data"""
    csv_file = os.path.join(market_data_dir, f"{ticker}_2025-01-02_to_{target_date}.csv")
    if not os.path.exists(csv_file):
        return None
    
    data = []
    with open(csv_file, 'r') as f:
        reader = csv.reader(f)
        for row in reader:
            if len(row) >= 6:
                ticker_name, date, open_price, high, low, close, volume = row
                # Skip header row
                if open_price == 'open':
                    continue
                data.append({
                    'ticker': ticker_name,
                    'date': date,
                    'open': float(open_price),
                    'high': float(high),
                    'low': float(low),
                    'close': float(close),
                    'volume': int(volume)
                })
    return data

def get_latest_vpa_signal(ticker, vpa_data_dir):
    """Extract the latest VPA signal from a ticker's VPA file"""
    vpa_file = os.path.join(vpa_data_dir, f"{ticker}.md")
    if not os.path.exists(vpa_file):
        return None
    
    with open(vpa_file, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Find the latest date entry
    date_pattern = r'- \*\*Ngày (\d{4}-\d{2}-\d{2}):\*\*'
    dates = re.findall(date_pattern, content)
    if not dates:
        return None
    
    latest_date = max(dates)
    
    # Extract the analysis for the latest date
    # Find the section starting with the latest date
    latest_section_pattern = rf'- \*\*Ngày {re.escape(latest_date)}:\*\*.*?(?=- \*\*Ngày|\Z)'
    match = re.search(latest_section_pattern, content, re.DOTALL)
    if not match:
        return None
    
    section_content = match.group(0)
    
    # Extract VPA signal from the analysis
    vpa_pattern = r'\*\*([^*]+)\*\*'
    vpa_signals = re.findall(vpa_pattern, section_content)
    
    # Filter for known VPA signals
    known_signals = [
        'Sign of Strength', 'SOS', 'Sign of Weakness', 'SOW',
        'Test for Supply', 'Test for Demand', 'Effort to Rise',
        'Effort to Fall', 'No Demand', 'No Supply', 'Stopping Volume',
        'Climax', 'Absorption', 'Support', 'Resistance'
    ]
    
    main_signal = None
    for signal in vpa_signals:
        if any(known in signal for known in known_signals):
            main_signal = signal
            break
    
    return {
        'date': latest_date,
        'signal': main_signal,
        'full_analysis': section_content.strip()
    }

def get_latest_monday():
    """Get the latest Monday date for weekly analysis"""
    today = datetime.now()
    # Find the most recent Monday
    days_since_monday = today.weekday()
    if days_since_monday == 0:  # Today is Monday
        # If it's early Monday, use last Monday
        if today.hour < 16:  # Assuming market closes at 4 PM
            latest_monday = today - timedelta(days=7)
        else:
            latest_monday = today
    else:
        latest_monday = today - timedelta(days=days_since_monday)
    
    return latest_monday.strftime('%Y-%m-%d')

def load_group_mappings(base_dir):
    """Load ticker-to-group mappings from GROUP.md"""
    group_file = os.path.join(base_dir, 'GROUP.md')
    mappings = {}
    
    if not os.path.exists(group_file):
        return mappings
    
    with open(group_file, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Parse the markdown table
    lines = content.strip().split('\n')
    for line in lines:
        if '|' in line and not line.startswith('|') and 'Ngành' not in line:
            parts = [p.strip() for p in line.split('|')]
            if len(parts) >= 3:
                sector = parts[1]
                tickers_str = parts[2]
                
                # Parse tickers
                tickers = [ticker.strip() for ticker in tickers_str.split(',')]
                for ticker in tickers:
                    if ticker:
                        mappings[ticker] = sector
    
    return mappings

def main():
    base_dir = '/Volumes/data/workspace/aipriceaction'
    market_data_dir = os.path.join(base_dir, 'market_data_week')
    vpa_data_dir = os.path.join(base_dir, 'vpa_data_week')
    
    # Load group mappings
    group_mappings = load_group_mappings(base_dir)
    
    # Get latest Monday for weekly analysis
    latest_monday = get_latest_monday()
    
    # Get all tickers from market_data_week
    tickers = []
    if os.path.exists(market_data_dir):
        for filename in os.listdir(market_data_dir):
            if filename.endswith('.csv'):
                ticker = filename.split('_')[0]
                tickers.append(ticker)
    
    tickers.sort()
    
    print(f"=== Weekly VPA Analysis Preparation Report for {latest_monday} ===")
    print(f"Total tickers found: {len(tickers)}")
    print(f"Weekly analysis target: Latest Monday ({latest_monday})")
    print()
    
    # Check each ticker
    need_analysis = []
    up_to_date = []
    missing_vpa = []
    
    for ticker in tickers:
        # Read market data
        market_data = read_csv_data(ticker, market_data_dir, latest_monday)
        if not market_data:
            continue
            
        # Check if latest Monday's data exists
        has_monday_data = any(row['date'] == latest_monday for row in market_data)
        if not has_monday_data:
            continue
            
        # Get latest VPA signal
        vpa_signal = get_latest_vpa_signal(ticker, vpa_data_dir)
        if not vpa_signal:
            missing_vpa.append(ticker)
            continue
            
        # Check if already has latest Monday's analysis
        if vpa_signal['date'] == latest_monday:
            up_to_date.append(ticker)
        else:
            # Extract current week and last 4 weeks of data
            current_week_data = next((row for row in market_data if row['date'] == latest_monday), None)
            
            # Get last 4 weeks of data (previous Mondays)
            last_4_weeks = []
            latest_monday_date = datetime.strptime(latest_monday, '%Y-%m-%d')
            for i in range(1, 5):  # Get previous 4 weeks
                target_date = (latest_monday_date - timedelta(weeks=i)).strftime('%Y-%m-%d')
                week_data = next((row for row in market_data if row['date'] == target_date), None)
                if week_data:
                    last_4_weeks.append(week_data)
            
            need_analysis.append({
                'ticker': ticker,
                'current_week_data': current_week_data,
                'last_4_weeks': last_4_weeks,
                'latest_vpa': vpa_signal
            })
    
    print(f"SUMMARY:")
    print(f"- Tickers needing new weekly VPA analysis: {len(need_analysis)}")
    print(f"- Tickers up-to-date: {len(up_to_date)}")
    print(f"- Tickers missing VPA files: {len(missing_vpa)}")
    print()
    
    if up_to_date:
        print(f"Tickers already up-to-date (have {latest_monday} entry):")
        for ticker in up_to_date:
            print(f"  - {ticker}")
        print()
    
    if missing_vpa:
        print("Tickers missing VPA files:")
        for ticker in missing_vpa:
            print(f"  - {ticker}")
        print()
    
    print("TICKERS NEEDING WEEKLY ANALYSIS:")
    print()
    
    for item in need_analysis:  # Show all tickers
        ticker = item['ticker']
        current_week_data = item['current_week_data']
        last_4_weeks = item['last_4_weeks']
        vpa = item['latest_vpa']
        
        # Get group info
        group = group_mappings.get(ticker, "UNKNOWN")
        
        print(f"=== {ticker} ({group}) ===")
        print(f"Current week data ({latest_monday}): O={current_week_data['open']}, H={current_week_data['high']}, L={current_week_data['low']}, C={current_week_data['close']}, V={current_week_data['volume']:,}")
        
        print("Last 4 weeks data:")
        for week_data in last_4_weeks:
            print(f"  {week_data['date']}: O={week_data['open']}, H={week_data['high']}, L={week_data['low']}, C={week_data['close']}, V={week_data['volume']:,}")
        
        print(f"Latest VPA signal ({vpa['date']}): {vpa['signal']}")
        print()
    
    print(f"Total tickers needing weekly analysis: {len(need_analysis)}")

if __name__ == "__main__":
    main()